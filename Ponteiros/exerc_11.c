...
VectorInt vectorint(){
    ...
    p->capacity = 1;
    p->nelements = 0;
    p->data = (int*)malloc((p->capacity)*sizeof(int));      //Aloca memoria para o ponteiro data
    if(p->data==NULL){
        printf("Memoria insuficiente!");
        exit(1);
    }
...
void vectorint_insert(VectorInt v, int a){
    if (v->capacity==v->nelements){     //Verifica se a capacidade do vetor dinamico é igual o numero de elementos ja inseridos
        v->data = realloc(v->data, (2*v->capacity)*sizeof(int));    //Realoca o dobro de capacidade de data
        if(v->data == NULL){
            printf("Memoria insuficiente!\n");
            exit(1);
        }
...
void vectorint_remove(VectorInt v, int a){
    ...
    if(v->nelements<v->capacity/2 && v->capacity>1){        //Verifica se o nelements é menor que metade da capacidade
        v->data = realloc(v->data, (v->capacity/2)*sizeof(int));    //Caso sim realoca data com metade da capacidade
        if(v->data == NULL){
            printf("Memoria insuficiente!\n");
            exit(1);
        }
...
int vectorint_at(VectorInt v, int i){       //Procura o i-esimo valor em data
...
char vectorint_pert(VectorInt v, int a){       //Procura o elemento a em data e retorna s ou n
    int i, j;
    for(i=0;i<v->nelements;i++){
        if(v->data[i]==a){
            return 's';
...
int main(){
    ...
    scanf("%d", &num);
    printf("%d pertence ao vetor? %c", num, vectorint_pert(dado, num));
